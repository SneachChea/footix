...
def multiplicative(odds: Union[List, np.ndarray], axis: int =-1) -> Tuple[np.ndarray, Union[float, np.ndarray]]:
    """
    ...
    if isinstance(odds, list):
        odds = np.array(odds)
    if len(odds.shape)>1:
        Z = np.sum(1./odds, axis=axis, keepdims=True)
    else:
        Z = np.sum(1./odds, axis=axis)
    margin = Z- 1.
    return 1./(Z*odds), margin
